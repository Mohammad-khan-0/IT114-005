...
public class Room implements AutoCloseable {
	...
	private boolean processCommands(String message, ServerThread client) {
		...
		if (message.equalsIgnoreCase("roll")) {
			...
			sendMessage(client,"Random number" + " " + random_int);
			return true;
        }
		...
		//Italics
		if(message.startsWith("!") && message.endsWith("!")){
			String italic = message.substring(1,message.length() -1); //length of string from start to finsih 
			String messageItalic = "<i>" + italic + "</i>";
			sendMessage(client, messageItalic);
			}
		
		//color
		if(message.startsWith("$") && message.endsWith("$")){
			String color = message.substring(1,message.length() -1); //length of string from start to finsih 
			String messageColor = " <font color = red > " + color + "</font>";
			sendMessage(client, messageColor);
		}

		//underline
		if(message.startsWith("_") && message.endsWith("_")){
			String underline = message.substring(1,message.length() -1); //length of string from start to finsih 
			String messageUnderline = "<u>" + underline + "</u>";
			sendMessage(client, messageUnderline);
		}
